...
def lemmaSentence(token_words):
    ...
    for word in token_words:
        tag = treetaggerwrapper.make_tags(tagger.tag_text(word), allow_extra=True)[0]
...
def stopWordRemovalDF(tweets_df):
    ...
    tweets_df["text"] = tweets_df["text"].apply(
        lambda x: stopWordRemoval(x, englishStopwords)
    )
    tweets_df["text"] = tweets_df["text"].apply(
        lambda x: stopWordRemoval(x, italianStopwords)
    )
    tweets_df["text"] = tweets_df["text"].apply(
        lambda x: stopWordRemoval2(x, stop_word)
    )
    return tweets_df
...
def changeRawWord(text, after, before):
    text = " ".join(text)
    text = re.sub(r"\b%s\b" % after, before, text)
    return word_tokenize(text)


def changeAllRawWord(tweets_df, path):
    changeList = pd.read_csv(path, names=["old", "new"])
    for index, row in changeList.iterrows():
        tweets_df["text"] = tweets_df["text"].apply(
            lambda x: changeRawWord(x, row["old"], row["new"])
        )
    return tweets_df
